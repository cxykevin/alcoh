...
var (
	// zeroWidth 覆盖的 Unicode 区间：组合记号与特殊零宽字符。
	zeroWidth = []runeRange{
		...
		{0x1DC0, 0x1DF9},
		{0x1DFB, 0x1DFF},
		{0x200B, 0x200F}, // 零宽空格、ZWJ(ZWNJ)、LTR/RTL 标记
		{0x202A, 0x202E}, // 嵌入/覆盖标记
		{0x2060, 0x2064}, // 字连接符等
		{0x2066, 0x206F},
		{0x20D0, 0x20F0}, // 组合用符号
		{0x2CEF, 0x2CF1},
	...
	wideRanges = []runeRange{
		...
		{0x1F18E, 0x1F18E},
		{0x1F191, 0x1F19A},
		{0x1F200, 0x1F202}, // 表情符号 (F)
		{0x1F210, 0x1F23B},
		{0x1F240, 0x1F248},
		{0x1F250, 0x1F251},
		{0x1F260, 0x1F265},
		{0x1F300, 0x1F320}, // 表情符号
		{0x1F32D, 0x1F335},
		{0x1F337, 0x1F37C},
		{0x1F37E, 0x1F393},
		{0x1F3A0, 0x1F3CA},
...
// runeWidth 返回单个 rune 占用的终端列宽（0/1/2）。
func runeWidth(r rune) int {
	switch {
	case r == '\t':
		return 1 // tab 由调用方按列展开
	case r < 0x20:
		return 0 // 控制字符不输出
...
// RuneWidth 导出单个 rune 的列宽。
func RuneWidth(r rune) int { return runeWidth(r) }

// StringWidth 返回字符串的总列宽（不含 tab 展开，tab 按 1 列计）。
...
// Truncate 按列宽截断 s 到 maxW 列。若发生截断，末尾补 "…"（占 1 列）。
func Truncate(s string, maxW int) string {
	...
	for _, r := range s {
		rw := runeWidth(r)
		if rw == 0 {
			continue // 零宽字符直接丢弃（截断语境）
		}
		if w+rw > maxW {
			// 用省略号替换末尾
			if w == maxW {
				// 正好占满，替换最后一个字符为省略号（若为宽字符需回溯）
				if len(out) > 0 && w > 1 {
					last := out[len(out)-1]
					if runeWidth(last) == 2 {
						out = out[:len(out)-1]
...
// Wrap 按 maxW 列宽换行，返回多行。宽字符不跨行断开。
func Wrap(s string, maxW int) []string {
	...
	for _, r := range s {
		...
		case r == '\n':
			flush()
		case rw == 0:
			line = append(line, r) // 保留零宽字符附着于前一个字符
		case w+rw > maxW:
			flush()
			line = append(line, r)
			w = rw
