...
func (ct *ConfigTree) draw(c *renderer.Canvas, r renderer.Rect, ed *model.ConfigEditor) {
	t := ct.Theme
	if ed == nil {
		c.PutText(r.X, r.Y, "正在获取服务端配置…", t.Style(t.TextMuted))
		return
	}
	y := r.Y
	...
	if ed.Saving {
		...
		c.PutText(r.X, bottomY, "保存中…  等待服务端确认后刷新", t.Style(t.TextMuted))
		return
	}
	if ed.Editing {
		prompt := "值: "
		if ed.EditNode != nil {
			prompt = "编辑 " + ed.EditNode.DisplayKey() + ": "
		}
		if inputY >= r.Y {
		...
		c.PutText(r.X, bottomY, "Esc 取消    Enter 保存", t.Style(t.TextMuted))
		return
	}
	if ed.AddingKey {
		if inputY >= r.Y {
			ib := &widget.InputBox{Buf: ed.AddInput, Prompt: "新键名: ", Style: t.Style(t.Text), Cursor: t.StyleOn(t.Background, t.Primary), Focused: true}
			ib.Draw(c, renderer.NewRect(r.X, inputY, r.W, 1))
		}
		c.PutText(r.X, bottomY, "Esc 取消    Enter 添加（map 键；对结构体字段无效会被服务端忽略）", t.Style(t.TextMuted))
		return
	}
	c.PutText(r.X, bottomY, "Esc 关闭   ↑↓ 选择   Enter 进入/编辑   ← 返回   r 刷新", t.Style(t.TextMuted))
}
