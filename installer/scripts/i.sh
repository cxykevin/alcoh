...
[0m         [47m  [0m            [47m  [0m     [0m
[0m         [47m  [0m                   [0m
[0m  [47m[8malcoh [0m [47m  [0m [47m      [0m [47m      [0m [47m    [0m
[0m  [47m  [0m  [47m  [0m [47m  [0m [47m  [0m     [1;44m  [0m  [47m  [0m   [47m  [0m
[0m  [47m   [0m [47m     [0m [47m             [0m   [47m  [0m
[0m[2m╭──────────────────────────────╮[0m
[0m[2m│ [0m[1;34maloch[0m[2m agent client installer │[0m
[0m[2m╰──────────────────────────────╯[0m
EOF
}
...
get_latest_release() {
    local api_url="https://api.github.com/repos/cxykevin/alcoh/releases"
    local max_retries=3
    local retry_delay=5
    local attempt=1
    local tag=""
...
select_package() {
    ...
    local pkg_manager=$2
    local tag=$3
    local base_url="https://github.com/cxykevin/alcoh/releases/download/$tag"
    ...
    case "$pkg_manager" in
        apt|apt-get)
            package="alcoh-linux-${arch}.deb"
            ;;
        dnf|yum)
            package="alcoh-linux-${arch}.rpm"
            ;;
        pacman)
            package="alcoh-linux-${arch}.pkg.tar.zst"
            ;;
        apk)
            package="alcoh-linux-${arch}.apk"
            ;;
        *)
...
download_and_install() {
    ...
    local tag=$3
    local package=$4
    local download_url="https://github.com/cxykevin/alcoh/releases/download/$tag/$package"
    local temp_dir="/tmp/alcoh_install"
    ...
    case "$package" in
        ...
        *.apk)
            ...
            ;;
        *)
            log_sub "安装 binary 到 /usr/bin/alcoh"
            sudo cp "$package" /usr/bin/alcoh
            sudo chmod +x /usr/bin/alcoh
            ;;
    esac
    cd /tmp
    rm -rf "$temp_dir"
