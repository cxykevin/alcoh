...
function Get-LatestRelease {
    $apiUrl = 'https://api.github.com/repos/cxykevin/alcoh/releases'
    $maxRetries = 3
    $retryDelay = 5
    $attempt = 1
...
function Install-MSIX {
    ...
    Write-LogSub "安装证书: ${PackagePath}.cer"
    ...
    try {
        ...
        Write-LogSubWarn "使用 -AllowUnsigned 失败: $($_.Exception.Message)"
...
function Main {
    ...
    $package = "alcoh-windows-amd64.msix"
    $downloadUrl = "https://github.com/cxykevin/alcoh/releases/download/$TAG/$package"
    $downloadCrtUrl = "https://raw.githubusercontent.com/cxykevin/.pubkey/refs/heads/main/msix-software.cer"
    Write-LogSub "安装包: $package"
    
    $tempDir = Join-Path $env:TEMP "alcoh_install"
    New-Item -ItemType Directory -Force -Path $tempDir | Out-Null
    $localPath = Join-Path $tempDir $package
    
    Write-LogMain "下载安装包"
    try {
        Write-LogSub "下载: $downloadCrtUrl "
    ...
    Write-LogMain "安装完成!"
    Write-LogSub "应用程序已安装，请从命令行启动 alcoh！"

    Write-LogMain "安装 Alkaid0..."
